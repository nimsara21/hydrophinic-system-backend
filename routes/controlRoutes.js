...
// Fetch the current system state from Firebase
router.get('/state', async (req, res) => {
  try {
    const snapshot = await admin.database().ref('systemControl').once('value');
    let systemState = snapshot.val();
    ...
    if (!systemState) {
      ...
      await admin.database().ref('systemControl').set(defaultState);
...
const toggleControl = async (controlKey) => {
  const snapshot = await admin.database().ref('systemControl').once('value');
  ...
  currentState[controlKey] = !currentState[controlKey];

  await admin.database().ref('systemControl').set(currentState);
  return currentState[controlKey];
};

// Toggle routes
router.post('/:controlKey', async (req, res) => {
  const controlKey = req.params.controlKey;
  try {
    const newState = await toggleControl(controlKey);
    res.json({ success: true, [controlKey]: newState });
  } catch (error) {
    console.error(`Error toggling ${controlKey}:`, error);
    res.status(500).json({ success: false, message: `Error toggling ${controlKey}` });
  }
});
